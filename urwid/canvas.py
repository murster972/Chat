...
class SolidCanvas(Canvas):
    ...
    def content(self, trim_left=0, trim_top=0, cols=None, rows=None, 
        ...
        cols = cols if isinstance(cols, int) else cols[0]
...
def shard_body_row(sbody):
    ...
    ** MODIFIES sbody by calling __next__() on its iterators **
    """
    row = []
    for done_rows, content_iter, cview in sbody:
        if content_iter:
            row.extend(content_iter.__next__())
        else:
...
def shards_delta(shards, other_shards):
    ...
    for num_rows, cviews in shards:
        if other_num_rows is None:
            other_num_rows, other_cviews = other_shards_iter.__next__()
...
def shard_cviews_delta(cviews, other_cviews):
    ...
    for cv in cviews:
        if other_cv is None:
            other_cv = other_cviews_iter.__next__()
...
def shards_join(shard_lists):
    ...
    shards_current = [i.__next__() for i in shards_iters]
        ...
        try:
            for i in range(len(shards_current)):
                if shards_current[i][0] > 0:
                    continue
                shards_current[i] = shards_iters[i].__next__()
        except StopIteration:
            break
    return new_shards
